...
?>
        ...
        <form id="excel_exp" action="admin.php?page=upicrm_export&action=excel_output" method="post">
            <input type="text" name="date_start2" class="datepicker" value="<?php echo upicrm_clean_data($_POST['date_start2']) ?>" placeholder="<?php _e('Start ','upicrm'); ?>" autocomplete="off">
            &nbsp;
            <input type="text" name="date_end2" class="datepicker" value="<?php echo upicrm_clean_data($_POST['date_end2']) ?>" placeholder="<?php _e('End ','upicrm'); ?>" autocomplete="off">
            &nbsp;
        ...
        <?php _e('Custom Export Link:','upicrm'); ?> <a href="<?php echo  $link ?>" target="_blank"><?php echo  $link; ?></a><br /><br />
        ...
        <?php _e('Example:','upicrm'); ?> <a href="<?php echo  $link ?>" target="_blank"><?php echo  $link; ?></a>
        <br /><br /><br />
        ...
        <form action="admin.php?page=upicrm_export&action=save_custom" method="post">
            <div id="upicrm_fields_order" style="display: none;">
                ...
                foreach ($UpiCRMFields->get_as_array(2) as $key => $value) { ?>
                    <input type="text" name="order[<?php echo  $i; ?>]" value="<?php echo  $key ?>" />
                <?php
                    $i++;
                } ?>
            </div>
            ...
            <ul id="upicrm_fields_sortable">
                <?php foreach ($UpiCRMFields->get_as_array(2) as $key => $value) { ?>
                    <li data-id="<?php echo  $key ?>" class="pointer-click">
                        <label>
                            <input type="checkbox" name="show[]" value="<?php echo $key ?>" <?php checked(isset($field_on[$key])); ?> />&nbsp;&nbsp;
