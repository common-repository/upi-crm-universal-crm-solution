...
Example: if subject contains buy, free, shop, cheap, new, deals – then change lead status to "Spam"','upicrm'); ?></li>
            ...
            <form method="post" action="admin.php?page=upicrm_lead_route">
                <?php if(isset($id)&&($id > 0)) { ?>
                    ...
                    <input type="hidden" name="upicrm_lead_id" value="<?php echo (int)$id; ?>" />
                <?php } else { ?>
                ...
                <select name="field_id"><?php
                                ...
                                foreach ($list_option as $key => $arr) {
                                    foreach ($arr as $key2 => $value) {
                                        ...
                                        ?><option value="<?php echo $key.'||exp||'.$key2; ?>" <?php echo $selected; ?> ><?php echo esc_attr($value); ?></option><?php
                                    }
                                }
                                ?></select>
                <select name="lead_route_type"><?php
                   ...
                   <?php } ?></select>
                : <input type="text" name="lead_route_value" value="<?php echo $id > 0 ?  esc_attr($GetLeadsRouteOBJ->lead_route_value) : ""?>" style="height: 28px; position: relative; top: 2px;" />
                ...
                <strong><?php _e('AND','upicrm'); ?></strong>
                               ...
                               <select name="field_id2"><?php
                                               ...
                                               foreach ($list_option as $key => $arr) {
                                                   foreach ($arr as $key2 => $value) {
                                                       ...
                                                       ?>
                                                       <option value="<?php echo $key.'||exp||'.$key2; ?>" <?php echo esc_attr($selected); ?> ><?php echo $value; ?></option>
                                                       <?php
                                                   }
                                               }
                                               ?></select>
                               ...
                               : <input type="text" name="lead_route_value2" value="<?php echo $id > 0 ?  esc_attr($GetLeadsRouteOBJ->lead_route_value2) : ""?>" style="height: 28px; position: relative; top: 2px;" />
                ...
                <strong><?php _e('AND','upicrm'); ?></strong>
                               ...
                               : <input type="text" name="lead_route_value3" value="<?php echo $id > 0 ?  esc_attr($GetLeadsRouteOBJ->lead_route_value3) : ""?>" style="height: 28px; position: relative; top: 2px;" />
                </div>
...
A lead will NOT be distributed to more than one person simultaneously!','upicrm'); ?>
                    ...
                    <li>
                        ...
                        <select name="lead_status_id">
                            ...
                            foreach ($UpiCRMLeadsStatus->get() as $status) {
                                ...
                                ?><option value="<?php echo $status->lead_status_id; ?>" <?php echo $selected; ?> ><?php echo esc_attr($status->lead_status_name); ?></option>
                           <?php } ?>
                        </select>
                    </li>
                    <li>
                        ...
                        <select name="change_field_id">
                            ...
                            foreach ($UpiCRMFields->get() as $field) {
                                ...
                                ?><option value="<?php echo $field->field_id; ?>" <?php echo $selected; ?> ><?php echo esc_attr($field->field_name); ?></option>
                           <?php } ?>
                        </select>
                        <?php _e('value to:','upicrm'); ?>
                        <input type="text" name="change_field_value" value="<?php echo $id > 0 ?  esc_attr($GetLeadsRouteOBJ->change_field_value) : ""?>" style="height: 28px; position: relative; top: 2px;"><br />
                        <?php _e('Note: You can use variables:','upicrm'); ?> [field-*], [url], [assigned-to], [lead-status], [affiliate-name]
                    ...
                    <li>
                        ...
                        <select name="integration_id">
                            ...
                            ?>
                                <option value="<?php echo $master->integration_id ?>" <?php selected( $GetLeadsRouteOBJ->integration_id, $master->integration_id) ?> ><?php echo esc_attr($master->integration_domain); ?></option>
                            <?php 
                                }
                            } ?>
                        </select>
                    ...
                    <li>
                        ...
                        <select name="mail_id">
                            ...
                            foreach ($getMails as $mail) {
                                ...
                                ?>
                                <option value="<?php echo  $mail->mail_id; ?>"  <?php echo $selected; ?>><?php echo esc_attr($mail->mail_event_name); ?></option>
                            <?php } ?>
                        </select>
                        <?php _e('to: ','upicrm'); ?>
                            ...
                            <?php
                                        ...
                                        foreach ($list_option as $key => $arr) {
                                            foreach ($arr as $key2 => $value) {
                                                ...
                                                if ( $key == "content") {
                                                ?><option value="<?php echo  $key2; ?>" <?php echo $selected; ?> ><?php echo esc_attr($value); ?></option><?php 
                                                }
                                            }
                                        }
                                        ?>
                ...
                <label for="lead_route_mail_no_cc"><?php _e('Don\'t send cc email','upicrm'); ?></label>
                    ...
                    <li>
                        <?php _e('Assign lead to affiliate','upicrm'); ?>
                        <select name="affiliate_id">
                            <option value="0"></option>
                            <?php foreach ($UpiCRMAffiliate->get_users() as $user) {
                                $selected = "";
                                if (isset($user->data->ID) && ($user->data->ID > 0)) {
                                    $selected = selected($GetLeadsRouteOBJ->affiliate_id, $user->data->ID, false);
                                }
                                ?>
                                <option value="<?php echo  $user->data->ID; ?>" <?php echo  $selected; ?>><?php echo  $user->data->display_name; ?></option>
                            <?php } ?>
                        </select>
                    </li>
                </ul>
                <br />
                &nbsp;&nbsp; <?php _e('Order','upicrm'); ?> <input type="number" step="1" name="lead_route_order" value="<?php echo $id > 0 ?  (int)$GetLeadsRouteOBJ->lead_route_order : "0"?>" style="height: 28px; position: relative; top: 2px;" />
