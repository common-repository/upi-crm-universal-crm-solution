...
//    
           ...
           //echo "console.log( 'Start: " . $lead_table_fields . "');";
            $lead_table_fields = str_replace("&quot;",'"',$lead_table_fields);
        ...
        function wp_ajax_remove_lead_callback() {
            $UpiCRMLeads = new UpiCRMLeads();
            $UpiCRMLeads->remove_lead((int)$_POST['lead_id']);
            die();
        }

        function wp_ajax_remove_lead_arr_callback() {
            $UpiCRMLeads = new UpiCRMLeads();
            $UpiCRMLeads->remove_leads((int)$_POST['lead_id']);
            die();
        }
        ...
        function wp_ajax_save_lead_callback() {
            ...
            $leadObj = $UpiCRMLeads->get_by_id((int)$_POST['lead_id']);
            $updateArr = array();
            ...
            if (isset($_POST['user_id'])) {
                if ($leadObj->user_id != $_POST['user_id']) {
                    $updateArr['user_id'] = (int)$_POST['user_id'];
                }
            }
            if (isset($_POST['lead_status_id'])) {
                if ($leadObj->lead_status_id != $_POST['lead_status_id']) {
                    $updateArr['lead_status_id'] = (int)$_POST['lead_status_id'];
                }
            }
            ...
            if ($leadObj->lead_management_comment != $_POST['remarks']) {
                $log->lead_management_comment = upicrm_clean_data($_POST['remarks']);
                $log->lead_id = (int)$_POST['lead_id'];
        ...
        function save_leads_arr() {
            ...
            $UpiCRMLeads = new UpiCRMLeads();
            $UpiCRMLeadsChangesLog = new UpiCRMLeadsChangesLog();
            $lead_status_id = (int)$_POST['lead_status_id_all'];
            $user_id = (int)$_POST['user_id_all'];
...
//                print_r($leadObj);
        ...
        function wp_ajax_send_lead_again_to_master_callback() {
            ...
            $UpiCRMIntegrationsLib->send_slave((int)$_POST['lead_id']);
            echo 1;
            die();
        }
        ...
        function wp_ajax_send_webservice_callback() {
            ...
            $UpiCRMWebServiceLib->send((int)$_POST['lead_id'], 0);
            $UpiCRMWebServiceLib->send((int)$_POST['lead_id'], 1);
            $UpiCRMWebServiceLib->send((int)$_POST['lead_id'], 2);
            echo 1;
            die();
        }
        ...
        function wp_ajax_get_lead_log_callback() {
            $UpiCRMLeadsChangesLog = new UpiCRMLeadsChangesLog();
            $logs = $UpiCRMLeadsChangesLog->get_by_lead_id((int)$_POST['lead_id']);
            if ($logs) {
                foreach ($logs as $log) {
        ...
        function wp_ajax_send_csv_again_callback() {
            $UpiCRMMails = new UpiCRMMails();
            $UpiCRMMails->send_csv((int)$_POST['lead_id']);
            echo 1;
            die();
        }
...
// add_action( 'wp_ajax_excel_output', array(new UpiCRMAdminAdminLists,'wp_ajax_excel_output_callback'));
    ...
    add_action('wp_ajax_upicrm_lead_table_fields', function () {
        $string = upicrm_clean_data($_POST['setting']);
